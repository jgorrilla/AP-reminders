...
private let googleSheetURL = ""

// MARK: - AP Course Models

struct APCourse: Identifiable {
    let id          = UUID()
    let name        : String
    let subject     : String   // "Math & CS", "Science", etc.
...
class CourseLoader: ObservableObject {
    @Published var courses      : [APCourse] = []
    @Published var isLoading    = true
    @Published var errorMessage : String?

    func load() {
        guard !googleSheetURL.isEmpty, let url = URL(string: googleSheetURL) else {
        ...
        URLSession.shared.dataTask(with: url) { data, _, error in
            DispatchQueue.main.async {
                ...
                if let error = error {
                    ...
                    self.courses = fallbackCourses(); return
                }
                guard let data = data,
                      let json = try? JSONSerialization.jsonObject(with: data) as? [[String: Any]]
                else {
                    self.errorMessage = "Could not parse sheet data — showing built-in list."
                    self.courses = fallbackCourses(); return
                }

                let loaded = json.compactMap { dict -> APCourse? in
                    guard let name        = dict["name"]         as? String,
                          let subject     = dict["subject"]       as? String,
                          let description = dict["description"]   as? String,
                          let prereqs     = dict["prerequisites"]  as? String,
                          let passRate    = dict["passRate"]       as? String,
                          let avgScore    = dict["avgScore"]       as? String,
                          let totalTakers = dict["totalTakers"]    as? String,
                          let difficulty  = dict["difficulty"]     as? String
                    else { return nil }
                    return APCourse(name: name, subject: subject, description: description,
                                    prerequisites: prereqs,
                                    examStats: APExamStats(passRate: passRate, avgScore: avgScore,
                                                           totalTakers: totalTakers, difficulty: difficulty))
                }

                self.courses = loaded.isEmpty ? fallbackCourses() : loaded
                if loaded.isEmpty { self.errorMessage = "Sheet returned no courses — showing built-in list." }
            }
        }.resume()
    }
}
...
struct Acorn: View {
    ...
    private var filteredCourses: [APCourse] {
        guard !searchText.isEmpty else { return loader.courses }
        return loader.courses.filter {
    ...
    private var grouped: [(subject: String, courses: [APCourse])] {
        let order = ["Math & CS", "Science", "History & Social Studies",
                     "English", "World Languages", "Arts", "Other"]
        return order.compactMap { subject in
            let list = filteredCourses.filter { $0.subject == subject }
            return list.isEmpty ? nil : (subject: subject, courses: list)
        }
    }
        ...
        NavigationStack {
            Group {
                if loader.isLoading {
                    VStack(spacing: 12) {
                        ProgressView()
                        Text("Loading courses…").font(.subheadline).foregroundColor(.secondary)
                    }
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                } else {
                    List {
                        // ── Error banner ──────────────────────
                        if let msg = loader.errorMessage {
                            Section {
                            ...
                            Button { openURL("https://www.youtube.com/watch?v=uGCUBSBgUGI") } label: {
                                HStack(spacing: 14) {
                                    ...
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text("Bluebook Tutorial").font(.body).foregroundColor(.primary)
                        ...
                        // ── My Classes summary ────────────────
                        if !appState.enrolledClasses.isEmpty {
                            Section {
                                let sorted = appState.enrolledClasses.sorted()
                                ForEach(sorted, id: \.self) { name in
                                    HStack {
                                        Text(name).font(.subheadline)
                                        Spacer()
                                        Button {
                                            appState.unenroll(from: name)
                                        } label: {
                                ...
                                Text("My Classes (\(appState.enrolledClasses.count))")
                            } footer: {
                                Text("You'll receive messages and calendar events for these classes.")
                            }
                        }
                        ...
                        Section {
                            ForEach(grouped, id: \.subject) { group in
                                ...
                                ForEach(group.courses) { course in courseRow(course) }
                            }
                        } header: {
    ...
    private func linkRow(title: String, subtitle: String, icon: String, color: Color, url: String) -> some View {
        Button { openURL(url) } label: {
            HStack(spacing: 14) {
                ZStack {
                    RoundedRectangle(cornerRadius: 10)
                        .fill(color.opacity(0.15)).frame(width: 44, height: 44)
    ...
    private func courseRow(_ course: APCourse) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Button {
                ...
                HStack {
                    // Enrolled dot indicator
                    if appState.isEnrolled(in: course.name) {
                        Circle().fill(Color.blue).frame(width: 8, height: 8)
                    }
                    Text(course.name).font(.body).foregroundColor(.primary).multilineTextAlignment(.leading)
                    Spacer()
            ...
            if expandedCourseID == course.id {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    // Enroll / unenroll button (students only)
                    if !appState.isTeacherMode {
                        let enrolled = appState.isEnrolled(in: course.name)
                        Button {
                            withAnimation {
                                if enrolled {
                                    appState.unenroll(from: course.name)
                                } else {
                                    appState.enroll(in: course.name)
                                }
                            }
                        } label: {
                            HStack {
                                Image(systemName: enrolled ? "checkmark.circle.fill" : "plus.circle.fill")
                                Text(enrolled ? "Remove from My Classes" : "Add to My Classes")
                            }
                            .font(.subheadline).fontWeight(.medium)
                            .foregroundColor(enrolled ? .red : .blue)
                        }
                        .buttonStyle(.plain)
                    }
                    ...
                    Label("Prerequisites", systemImage: "checkmark.seal")
                        .font(.caption).foregroundColor(.secondary)
                    Text(course.prerequisites).font(.subheadline)

                    // Exam stats sub-dropdown
                    Button {
...
// ============================================================
// MARK: - Built-in Course List
// ============================================================
// To ADD a course:    append a new APCourse(...) entry below.
// To REMOVE a course: delete its APCourse(...) entry below.
// Subject must be one of:
//   "Math & CS" | "Science" | "History & Social Studies" |
//   "English"   | "World Languages" | "Arts" | "Other"
// ============================================================

private func fallbackCourses() -> [APCourse] { [

    // ── Math & Computer Science ──────────────────────────────

    APCourse(name: "AP Calculus AB", subject: "Math & CS",
    ...
    APCourse(name: "AP Calculus BC", subject: "Math & CS",
    ...
    APCourse(name: "AP Statistics", subject: "Math & CS",
    ...
    APCourse(name: "AP Computer Science A", subject: "Math & CS",
    ...
    APCourse(name: "AP Computer Science Principles", subject: "Math & CS",
    ...
    APCourse(name: "AP Precalculus", subject: "Math & CS",
    ...
    APCourse(name: "AP Biology", subject: "Science",
    ...
    APCourse(name: "AP Chemistry", subject: "Science",
    ...
    APCourse(name: "AP Physics 1", subject: "Science",
    ...
    APCourse(name: "AP Physics 2", subject: "Science",
    ...
    APCourse(name: "AP Physics C: Mechanics", subject: "Science",
    ...
    APCourse(name: "AP Physics C: Electricity & Magnetism", subject: "Science",
    ...
    APCourse(name: "AP Environmental Science", subject: "Science",
    ...
    APCourse(name: "AP U.S. History", subject: "History & Social Studies",
    ...
    APCourse(name: "AP World History: Modern", subject: "History & Social Studies",
    ...
    APCourse(name: "AP European History", subject: "History & Social Studies",
    ...
    APCourse(name: "AP Government & Politics: U.S.", subject: "History & Social Studies",
    ...
    APCourse(name: "AP Government & Politics: Comparative", subject: "History & Social Studies",
    ...
    APCourse(name: "AP Human Geography", subject: "History & Social Studies",
    ...
    APCourse(name: "AP Psychology", subject: "History & Social Studies",
    ...
    APCourse(name: "AP Economics: Macro", subject: "History & Social Studies",
    ...
    APCourse(name: "AP Economics: Micro", subject: "History & Social Studies",
    ...
    APCourse(name: "AP English Language & Composition", subject: "English",
    ...
    APCourse(name: "AP English Literature & Composition", subject: "English",
    ...
    APCourse(name: "AP Spanish Language & Culture", subject: "World Languages",
    ...
    APCourse(name: "AP Spanish Literature & Culture", subject: "World Languages",
    ...
    APCourse(name: "AP French Language & Culture", subject: "World Languages",
    ...
    APCourse(name: "AP Chinese Language & Culture", subject: "World Languages",
    ...
    APCourse(name: "AP Japanese Language & Culture", subject: "World Languages",
    ...
    APCourse(name: "AP German Language & Culture", subject: "World Languages",
    ...
    APCourse(name: "AP Italian Language & Culture", subject: "World Languages",
    ...
    APCourse(name: "AP Latin", subject: "World Languages",
    ...
    APCourse(name: "AP Art History", subject: "Arts",
    ...
    APCourse(name: "AP Music Theory", subject: "Arts",
    ...
    APCourse(name: "AP Studio Art: 2-D Design", subject: "Arts",
    ...
    APCourse(name: "AP Studio Art: 3-D Design", subject: "Arts",
    ...
    APCourse(name: "AP Studio Art: Drawing", subject: "Arts",
    ...
    APCourse(name: "AP Seminar", subject: "Other",
    ...
    APCourse(name: "AP Research", subject: "Other",
